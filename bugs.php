...
<?php
  $tableName = 'bug';
  $columnScheme = "*";
  $whereValue = "state > 0 AND state < 3 ORDER BY id DESC";
...
<?php
  $tableName = 'bug';
  $columnScheme = "*";
  $whereValue = "state >= 3 ORDER BY id DESC";
  $result = dbQuery($connection, $tableName, $columnScheme, $whereValue);
  if ( mysqli_num_rows($result) > 0 ) {
    echo "<table class=\"table\">";
    echo "<thead><tr><th scope=\"col\">#Lp.</th><th scope=\"col\">#id</th><th scope=\"col\">Produkt</th><th scope=\"col\">Komponent</th>";
    ...
    echo "<div class=\"alert alert-primary\" role=\"alert\">Nie znaleziono żadnych zamkniętych zgłoszeń</div>";
  }
?>
  </div>
</div>
